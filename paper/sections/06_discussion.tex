...
the same brittleness reported by RAID~\cite{dugan2024raid} and
MAGE~\cite{li2024mage}; the quantitative shape (huge AUC--accuracy gap)
is a useful signal for practitioners who only ever look at AUC.
...
\subsection{Calibration as a Cross-Domain Fix}
\label{ssec:calibration_fix}

The AUC--accuracy gap on RAID has a straightforward post-hoc remedy:
temperature scaling~\cite{guo2017calibration} fits a single scalar \(T\)
on a held-out calibration set (e.g., the validation split) so that
$p_{\text{cal}} = \mathrm{softmax}(\mathrm{logits}/T)$. When \(T > 1\)
the model is over-confident; scaling moves probabilities toward 0.5 and
recalibrates the threshold. We implement this in
\texttt{evaluation/calibration.py}, which additionally sweeps the threshold
grid on the calibration set to find the $\theta^*$ that maximises
validation \(F_1\). Even without retraining, replacing the fixed
\(\theta = 0.5\) threshold with $\theta^*$ can meaningfully close the
accuracy gap on cross-domain data.
