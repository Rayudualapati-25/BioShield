...
def _append_metrics_row(cfg: dict, row: dict) -> None:
    ...
    header = ["round", "condition", "auc", "f1", "evasion_rate", "bertscore_f1", "mean_perplexity"]
    exists = path.exists()
...
def run_static_baseline(cfg: dict, config_path: str) -> dict:
    ...
    _run([
        sys.executable, "models/detector/train_detector.py",
        "--config", config_path, "--mode", "baseline", "--label", label,
    ])
    metrics_path = Path(cfg["paths"]["metrics_dir"]) / f"{label}_metrics.json"
    payload = json.loads(metrics_path.read_text(encoding="utf-8"))
    _append_metrics_row(cfg, {
        "round": 0, "condition": label,
        "auc": payload["test"]["auc"], "f1": payload["test"]["f1"],
        "evasion_rate": "", "bertscore_f1": "", "mean_perplexity": "",
    })
    return payload
...
def _score_detector_on(cfg: dict, detector_ckpt: Path, test_csv: Path, device) -> dict:
    ...
    ds = TextClassificationDataset(str(test_csv), tokenizer, int(cfg["detector"]["max_length"]))
...
def _build_round_test_set(cfg: dict, round_idx: int, fakes_csv: Path, test_csv_out: Path) -> None:
    ...
    merged = pd.concat([real.sample(n=n, random_state=round_idx), fakes.sample(n=n, random_state=round_idx)], ignore_index=True)
    merged = merged[["text", "label"]]
    test_csv_out.parent.mkdir(parents=True, exist_ok=True)
    merged.to_csv(test_csv_out, index=False)


def run_loop(condition: str, cfg: dict, config_path: str) -> dict:
    ...
    _run([
        sys.executable, "models/detector/train_detector.py",
        "--config", config_path, "--mode", "baseline", "--label", base_label,
    ])
    ...
    for r in range(1, num_rounds + 1):
        ...
        if condition == "seqgan_only":
            ...
            if not ckpt.exists():
                _run([sys.executable, "models/seqgan/train_seqgan.py", "--config", config_path])
            _run([
                sys.executable, "models/seqgan/train_seqgan.py",
                "--config", config_path, "--generate_only",
                "--checkpoint", str(ckpt), "--n", str(pool_size),
                "--out", str(fakes_csv),
            ])
        else:
            ...
            if not (gen_ckpt / "config.json").exists():
                _run([sys.executable, "models/generator/train_generator.py", "--config", config_path])
            _run([
                sys.executable, "models/generator/train_generator.py",
                "--config", config_path, "--generate_only",
                "--checkpoint", str(gen_ckpt), "--n", str(pool_size),
                "--out", str(fakes_csv),
            ])
        ...
        LOG.info(
            ...
            r, label, scored["auc"], scored["f1"], scored["evasion_rate"],
        )
        _append_metrics_row(cfg, {
            "round": r, "condition": label,
            "auc": scored["auc"], "f1": scored["f1"],
        ...
        if condition in ("full_pipeline", "seqgan_only"):
            ...
            pd.concat([base_train, more_fakes], ignore_index=True).to_csv(aug_train, index=False)
            ...
            _run([
                sys.executable, "models/detector/train_detector.py",
                "--config", str(round_cfg), "--mode", "retrain",
                "--resume", str(current_ckpt), "--label", round_ckpt_label,
            ])
...
def main() -> None:
    ...
    if args.condition == "static_baseline":
        ...
        run_loop(args.condition, cfg, args.config)
